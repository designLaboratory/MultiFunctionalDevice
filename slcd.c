...
void slcdInitialize(void){	
	...
	//all ports necessary to configure LED
 ...
 *----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------
  Function for  displaying dots 
 *----------------------------------------------------------------------------*/
void slcdSetDoubleDot(void) {
			//LCD->WF8B[LCD_Front_Pin[((2*4)-2)]] |= (LCD_C);
			LCD->WF8B[LCD_Front_Pin[((2*4)-1)]] |= (LCD_S_DEC);
}

void slcdSetDot(uint8_t ktora) {                           
			//LCD->WF8B[LCD_Front_Pin[((2*ktora)-2)]] |= (LCD_C);
			LCD->WF8B[LCD_Front_Pin[((2*ktora)-1)]] |= (LCD_S_DEC);
}
...
void slcdSet(uint8_t value, uint8_t digit){	
	...
	switch(value){
		...
		case 0x0A:
			...
			break;
		case 0x0B:
			LCD->WF8B[LCD_Front_Pin[((2*digit)-2)]] = (LCD_S_D | LCD_S_F | LCD_S_E | LCD_S_G);
 ...
 *----------------------------------------------------------------------------*/
void slcdDisplay(uint32_t value ,uint16_t format){	
			uint16_t tab[] = {0,0,0,0};
			uint8_t i = 0;
			while(value) {	
				tab[i++]=value%format;
				value/=format;
